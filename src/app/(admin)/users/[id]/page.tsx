...
export default async function UserDetailPage({
  ...
  const [
    ...
    db
      .from("user_badges")
      .select("badge_id, unlocked_at, badges!inner(title, icon_url)")
      .eq("user_id", id)
      .order("unlocked_at", { ascending: false }),
    db
      .from("badges")
      .select("id, title, requirement_value")
      .eq("is_active", true)
  ...
  const awardable = (
    (allBadges ?? []) as Pick<Badge, "id" | "title" | "requirement_value">[]
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <Card title="Earned badges">
          <ul className="space-y-2">
            {(earned ?? []).map((e) => {
              const badge = e.badges as unknown as {
                title: string;
                icon_url: string | null;
              };
              return (
                <li
                  key={e.badge_id as string}
          ...
          {awardable.length > 0 && (
            <form
              action={awardBadge}
              className="mt-5 flex items-end gap-2 border-t border-neutral-100 pt-4"
            >
        ...
        <Card title="Subscription">
          <div className="text-sm text-neutral-700">
            <p>
              Plan: <strong>{subscription?.plan ?? "free"}</strong> &middot;
              Status: <strong>{subscription?.status ?? "active"}</strong>
            </p>
